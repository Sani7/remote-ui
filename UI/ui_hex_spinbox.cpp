...
UI_hex_spinbox::UI_hex_spinbox(double min, double max, double value, QObject* parent) : UI_item(UI_HEX_SPINBOX_TYPE, parent)
{
    m_min = min;
    m_max = max;
    m_value = value;
...
void UI_hex_spinbox::from_json(const json &j)
{
    UI_item::from_json(j);
...
json UI_hex_spinbox::to_json(size_t id) const
{
