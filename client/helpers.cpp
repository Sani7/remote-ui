...
void create_dial_needle(QwtDial *dial)
{
    QwtDialSimpleNeedle *needle =
        new QwtDialSimpleNeedle(QwtDialSimpleNeedle::Arrow, true, Qt::yellow, QColor(Qt::gray).lighter(130));
    needle->setWidth(2);
...
{
    QString style =
        QString("#%1 { background-color: %2; color: %3;}").arg(widget->objectName(), bg.name(), color.name());
