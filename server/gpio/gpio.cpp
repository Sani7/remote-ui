...
int gpiod_line_config_add_line_settings(struct gpiod_line_config *config, const unsigned int *offsets,
...
struct gpiod_line_request *gpiod_chip_request_lines(struct gpiod_chip *chip, struct gpiod_request_config *req_cfg,
...
void GPIO::configure_pin(Direction dir, uint8_t gpio, bool state = false)
{
    ...
    if (dir == Direction::Output)
    {
        ...
        if (!m_request)
        {
            SPDLOG_ERROR("failed to request line: {}", strerror(errno));
        }
        return;
    }
...
struct gpiod_line_request *GPIO::request_output_line(const char *chip_path, unsigned int offset,
...
struct gpiod_line_request *GPIO::request_input_line(const char *chip_path, unsigned int offset, const char *consumer)
{
    struct gpiod_request_config *req_cfg = NULL;
    struct gpiod_line_request *request = NULL;
    struct gpiod_line_settings *settings;
