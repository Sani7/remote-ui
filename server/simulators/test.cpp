...
Test_Sim::Test_Sim(Communication *com, QObject *parent)
    : Simulator_base("Test Sim", com, std::chrono::milliseconds(1000), parent),
      ...
      m_combobox({"Option 1", "Option 2", "Option 3"}, 0),
      m_label("Label", Color::White, Color::Black), m_checkbox("Option 1", Color::Default, Color::Default),
      m_slider("Slider", 0, 100, 0), m_dial("Dial", 0, 100, 0),
      m_thermo("Thermo", "%", 0, 100, 0), m_led("Led", Color::White, Color::Red),
      ...
      m_tab_widget({"Tab 1", "Tab 2", "Tab 3"}, 0), m_stacked_widget(3, 0), m_status_bar()
{
    ...
    {
        m_table.insert_item(i / 4, i % 4, QString("%0 %1").arg(i / 4).arg(i % 4));
    }
...
void Test_Sim::slider_changed(UI_slider *id, double value)
{
    if (id == &m_slider)
    {
        ...
        m_label.set_text(QString::number(value));
        ...
        m_status_bar.show_message("Test slider changed to " + QString::number(value));
        return;
    }
}
